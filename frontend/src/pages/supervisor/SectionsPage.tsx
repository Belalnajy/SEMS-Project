...
export default function SectionsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editItem) {
        await api.put(`/sections/${editItem.id}`, form);
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.delete(`/sections/${showConfirmDelete}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1">إدارة الفصول</h1>
          <p className="text-sm text-slate-400">
            إضافة وتعديل وحذف الفصول الدراسية
          </p>
        </div>
        <button
          ...
          <HiOutlinePlus className="h-5 w-5" /> إضافة فصل
        </button>
      </div>
      ...
      <ConfirmModal
        ...
        title="حذف فصل"
        message="هل أنت متأكد من حذف هذا الفصل؟"
        isDanger={true}
        loading={isDeleting}
      />
      ...
      <Modal
        ...
        title={editItem ? 'تعديل الفصل' : 'إضافة فصل جديد'}
        maxWidth="max-w-md">
